...
exp = Experiment('polar_AM_frierson_insolation_newbucket_0qflux_samehcp_landocean_commit'+commit_nr, codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/polar_AM/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
...
exp = Experiment('polar_AM_frierson_insolation_newbucket_0qflux_samehcp_landocean_plus_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/polar_AM/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'polar_AM_frierson_insolation_newbucket_0qflux_samehcp_landocean_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
