...
# MP added for vegetation project: narrow continents -- narrow_lonspan = e.g. _three / _six / _twelve / _twentyfour 

    elif land_mode=='narrow_twentyfour': 
        idx = np.zeros((nlat,nlon), dtype=bool) # MP added for two continents
        idx = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (0. <= lon_array) & (24. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.
...
# Next produce a topography array
    ...
    topo_filename = GFDL_BASE + '/input/narrow_twentyfour/land.nc' 
    print(topo_filename)
...
if __name__ == "__main__":
    ...
    write_land('test',land_mode='narrow_twentyfour')
