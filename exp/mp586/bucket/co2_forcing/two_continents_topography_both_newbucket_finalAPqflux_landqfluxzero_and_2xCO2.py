...
exp = Experiment('two_continents_topography_both_newbucket_finalAPqflux_landqfluxzero', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents_topo_both/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/two_continents_topo_both/ocean_qflux.nc')]
...
####### co2 experiment #######
exp = Experiment('two_continents_topography_both_newbucket_finalAPqflux_landqfluxzero_2xCO2_spinup_361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents_topo_both/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/two_continents_topo_both/ocean_qflux.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file='/scratch/mp586/Isca_DATA/two_continents_topography_both_newbucket_finalAPqflux_landqfluxzero/restarts/res0361.tar.gz', num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
