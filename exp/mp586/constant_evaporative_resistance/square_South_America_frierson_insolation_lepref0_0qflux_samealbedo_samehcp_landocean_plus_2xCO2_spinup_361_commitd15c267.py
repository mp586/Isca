...
exp = Experiment('square_South_America_frierson_insolation_lepref0_0qflux_samealbedo_samehcp_landocean_commit'+commit_nr, codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/square_South_America/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
...
#        'max_bucket_depth_land':0.5, #Set max bucket depth over land default = 0.15
    ...
    'surface_flux_nml': {
        ...
        'land_evap_prefactor': 0. # land evap = potential evap*0.7    
...
exp = Experiment('square_South_America_frierson_insolation_lepref0_0qflux_samealbedo_samehcp_landocean_plus_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/square_South_America/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'square_South_America_frierson_insolation_lepref0_0qflux_samealbedo_samehcp_landocean_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
