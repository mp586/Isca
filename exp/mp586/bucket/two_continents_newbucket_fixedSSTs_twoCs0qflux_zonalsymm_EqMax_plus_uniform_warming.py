...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='6700769')
...
# prescribed SSTs with climatology from Isca/two_continents_newbucket_fullnewbucketqflux
exp = Experiment('two_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax', codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/ssts_zonsymm_eqmax.nc')]
...
#        'max_bucket_depth_land':0.5, #Set max bucket depth over land default = 0.15
    ...
    'mixed_layer_nml': {
        ...
        'sst_file' : 'ssts_zonsymm_eqmax', #Set name of sst input file
...
exp = Experiment('two_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_plus_2pt52K_and_2xCO2_spinup_361', codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/ssts_zonsymm_eqmax_unif_warmg.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
...
#        'max_bucket_depth_land':0.5, #Set max bucket depth over land default = 0.15
    ...
    'mixed_layer_nml': {
        ...
        'do_read_sst' : True, #Read in sst values from input file
        'do_sc_sst' : True, #Do specified ssts (need both to be true)
        'sst_file' : 'ssts_zonsymm_eqmax_unif_warmg',
        'specify_sst_over_ocean_only' : True, #Make sure sst only specified in regions of ocean.
    },
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'two_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
