...
# cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='b3c4afc')
...
# exp = Experiment('full_continents_newbucket_fixedSSTs_zonally_symmetric', codebase=cb)
...
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_daily', 15, 'days', time_units='days')
...
diag.add_field('atmosphere', 'bucket_depth', time_avg=False)
diag.add_field('atmosphere', 'bucket_depth_cond', time_avg=False)
diag.add_field('atmosphere', 'bucket_depth_conv', time_avg=False)
diag.add_field('atmosphere', 'bucket_depth_lh', time_avg=False)
...
diag.add_field('dynamics', 'height', time_avg=False) # geopotential height 
diag.add_field('atmosphere', 'rh', time_avg=False) 
diag.add_field('dynamics', 'slp', time_avg=False) # sea level pressure
diag.add_field('dynamics', 'zsurf', time_avg=False) # geopotential height at surface
diag.add_field('rrtm_radiation', 'toa_sw',time_avg=False)

diag.add_field('rrtm_radiation', 'flux_sw', time_avg=False) # net SW surface flux
diag.add_field('rrtm_radiation', 'flux_lw', time_avg=False) # net LW surface flux
diag.add_field('mixed_layer', 'flux_lhe', time_avg=False) # latent heat flux (up) at surface
diag.add_field('mixed_layer', 'flux_t', time_avg=False) # sensible heat flux (up) at surface
diag.add_field('rrtm_radiation', 'co2', time_avg=False)

diag.add_field('dynamics', 'sphum_u', time_avg=False)
diag.add_field('dynamics', 'sphum_v', time_avg=False)
diag.add_field('dynamics', 'sphum_w', time_avg=False)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'full_continents_newbucket_fixedSSTs_zonally_symmetric/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
...
# # Do CO2 run with prescribed SSTS from Isca/full_continents_newbucket_fullnewbucketqflu_2xCO2_spinup_361 starting from spun-up state of the run above

# exp = Experiment('full_continents_newbucket_fixedSSTs_from_experiment_full_continents_newbucket_fullnewbucketqflux_2xCO2_spinup361', codebase=cb)

# #Add any input files that are necessary for a particular experiment.
# exp.inputfiles = [os.path.join(GFDL_BASE,'input/all_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/all_continents/prescribed_ssts_perturbed.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
# #Tell model how to write diagnostics
# diag = DiagTable()
# diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
# #Lets do a run!
# exp.run(1, restart_file=os.path.join(GFDL_DATA,'full_continents_newbucket_fixedSSTs_from_experiment_full_continents_newbucket_fullnewbucketqflux/restarts/res0361.tar.gz'), num_cores=NCORES)
# for i in range(2,481):
#     exp.run(i, num_cores=NCORES)
