...
exp = Experiment('two_continents_topography_both_oceantopogsmoothing08_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral', codebase=cb)
...
####### co2 experiment #######
exp = Experiment('two_continents_topography_both_oceantopogsmoothing08_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_2xCO2_spinup_361', codebase=cb)
...
#Lets do a run!
exp.set_resolution('T42', 40)
exp.run(1, restart_file=os.path.join(GFDL_DATA,'two_continents_topography_both_oceantopogsmoothing08_newbucket_finalIscaAPqflux_landqfluxzero/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
