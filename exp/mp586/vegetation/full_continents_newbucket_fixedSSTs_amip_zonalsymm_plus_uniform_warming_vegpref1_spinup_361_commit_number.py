...
exp = Experiment('full_continents_newbucket_fixedSSTs_zonally_symmetric_vegetation_vegpref1_commit_'+commit_nr, codebase=cb)
...
exp = Experiment('full_continents_newbucket_fixedSSTs_zonally_symmetric_vegetation_vegpref1_plus_2pt52K_and_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)
...
#        'max_bucket_depth_land':0.5, #Set max bucket depth over land default = 0.15
    ...
    'surface_flux_nml': {
        'veg_evap_prefactor': 1. # vegetation at 600 ppm co2 reduces evaporation by 80% -- just a suggestion 
    }
})


#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'full_continents_newbucket_fixedSSTs_zonally_symmetric_vegetation_vegpref1_commit_'+commir_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
