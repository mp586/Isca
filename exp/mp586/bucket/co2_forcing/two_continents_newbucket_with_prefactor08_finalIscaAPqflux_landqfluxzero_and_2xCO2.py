...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='5fdaad7')
...
exp = Experiment('two_continents_newbucket_with_prefactor08_finalIscaAPqflux_landqfluxzero', codebase=cb)
...
####### co2 experiment #######
exp = Experiment('two_continents_newbucket_with_prefactor08_finalIscaAPqflux_landqfluxzero_2xCO2_spinup_361', codebase=cb)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'two_continents_newbucket_with_prefactor08_finalIscaAPqflux_landqfluxzero/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
