...
def write_land(exp,land_mode='square',boundaries=[-30.,30.,0.,100.],continents=['all'],topo_mode='both',mountains=['all'],topo_ellipse1=[-15.,5.,15.,8.,4500.],topo_ellipse2=[-10.,125.,25.,10.,3000.],waterworld=False):
...
# MP added for two continents, short South America 

    elif land_mode=='two_continents_short_South_America': 
        idx_c = np.zeros((2,nlat,nlon), dtype=bool) # MP added for two continents
        idx_c[0,:,:] = (boundaries[0] <= lat_array) & (lat_array <= 0.) & (0. <= lon_array) & (40. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx_c[1,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (80. <= lon_array) & (140. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx = idx_c[0,:,:] + idx_c[1,:,:]
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.
...
# Next produce a topography array
    ...
    topo_filename = GFDL_BASE + '/input/two_continents_short_South_America_topo_both/land.nc' 
    print(topo_filename)
...
if __name__ == "__main__":
    ...
    write_land('test',land_mode='two_continents_short_South_America')
