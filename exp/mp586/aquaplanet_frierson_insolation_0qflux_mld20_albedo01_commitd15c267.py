...
from isca import IscaCodeBase, DiagTable, Experiment, Namelist, GFDL_BASE, GFDL_WORK, GFDL_DATA
...
exp = Experiment('aquaplanet_frierson_insolation_0qflux_mld20_albedo01_commit'+commit_nr, codebase=cb)

input_path = os.path.join(GFDL_WORK,'codebase/https___github.com_mp586_Isca.git-'+commit_nr+'/code/')


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/aquaplanet/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc')]
...
exp = Experiment('aquaplanet_frierson_insolation_0qflux_mld20_albedo01_plus_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/aquaplanet/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'aquaplanet_frierson_insolation_0qflux_mld20_albedo01_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
