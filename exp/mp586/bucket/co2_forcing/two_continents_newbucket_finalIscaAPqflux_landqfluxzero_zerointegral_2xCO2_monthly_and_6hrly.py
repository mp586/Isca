...
exp = Experiment('two_continents_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly', codebase=cb)
...
####### co2 experiment #######
exp = Experiment('two_continents_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly_2xCO2_spinup_361', codebase=cb)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'two_continents_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
