...
# exp = Experiment('narrow_twentyfour_newbucket_fixedSSTs_from_realworld_zonallysymm_commit'+commit_nr, codebase=cb)


# #Add any input files that are necessary for a particular experiment.
# exp.inputfiles = [os.path.join(input_path,'input/narrow_twentyfour/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/sst_clim_amip_zonalsymm.nc')]
# #Tell model how to write diagnostics
# diag = DiagTable()
...
exp = Experiment('narrow_twentyfour_newbucket_fixedSSTs_from_realworld_zonallysymm_plus_uniform_warming_and_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/narrow_twentyfour/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/amip_zonsymm_uniform_warming.nc'), os.path.join(input_path,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'narrow_twentyfour_newbucket_fixedSSTs_from_realworld_zonallysymm_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
