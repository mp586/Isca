...
#careful, need to set land mode in function call at the bottom of this script! 


def write_land(exp,land_mode='square',boundaries=[-30.,30.,0.,100.],continents=['all'],topo_mode='none',mountains=['all'],topo_ellipse1=[-15.,5.,15.,8.,4500.],topo_ellipse2=[-10.,125.,25.,10.,3000.],waterworld=False):
...
# Common features of set-ups
    ...
    lon_array, lat_array = np.meshgrid(lons,lats) # lat_array[:,0]=lats
...
# Firstly determine the land set-up to be used    
    ...
    if land_mode=='square': # produces only ONE square of land between 20N and 60N, and between 200W and 60W, corresponds to NorthEast Africa + Eurasia
        idx = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (boundaries[2] <= lon_array) & (boundaries[3] >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.
    

# MP added for two continents

    elif land_mode=='two_continents': 
        idx_c = np.zeros((2,nlat,nlon), dtype=bool) # MP added for two continents
        idx_c[0,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (0. <= lon_array) & (40. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx_c[1,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (80. <= lon_array) & (140. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx = idx_c[0,:,:] + idx_c[1,:,:]
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.


    elif land_mode=='two_continents_100apart_at20lon': 
        idx_c = np.zeros((2,nlat,nlon), dtype=bool) # MP added for two continents
        ...
        idx_c[1,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (160. <= lon_array) & (220. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx = idx_c[0,:,:] + idx_c[1,:,:]
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.

    elif land_mode=='two_continents_10apart_at20lon': 
        idx_c = np.zeros((2,nlat,nlon), dtype=bool) # MP added for two continents
        idx_c[0,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (20. <= lon_array) & (60. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx_c[1,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (70. <= lon_array) & (130. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx = idx_c[0,:,:] + idx_c[1,:,:]
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.

    elif land_mode=='two_continents_70apart_at20lon': 
        idx_c = np.zeros((2,nlat,nlon), dtype=bool) # MP added for two continents
        idx_c[0,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (20. <= lon_array) & (60. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx_c[1,:,:] = (boundaries[0] <= lat_array) & (lat_array <= boundaries[1]) & (130. <= lon_array) & (190. >= lon_array) # boolean with shape of lat array = (64,128) #MP May2017 added equal signs (<=, =>)
        idx = idx_c[0,:,:] + idx_c[1,:,:]
        land_array[idx] = 1.0 # outs ones everywhere where idx = true, so in between the defined boundaries.
...
# Next produce a topography array
    if topo_mode == 'none':
        ...
        #Options to define simple Gaussian Mountain
        central_lat1 = topo_ellipse1[0]
        central_lon1 = topo_ellipse1[1]
        radius_degrees1 = topo_ellipse1[2]
        std_dev1 = topo_ellipse1[3]
        height1 = topo_ellipse1[4]

        ax_rot = 10. #gradient of new x axis
        ax_rat = 40. #axis ratio a**2/b**2
        rsqd_array1 = np.sqrt((lon_array - central_lon1 + ax_rot*(lat_array - central_lat1))**2.+ ax_rat*(lat_array - central_lat1 - ax_rot*(lon_array - central_lon1))**2.)*np.cos(np.arctan(ax_rot))

        idx1 = (rsqd_array1 < radius_degrees1) 
        topo_array[idx1] = height1* np.exp(-(rsqd_array1[idx1]**2.)/(2.*std_dev1**2.))

        ax_rot = 10. #gradient of new x axis
        ax_rat = 4. #axis ratio a**2/b**2
        central_lat2 = topo_ellipse2[0]
        central_lon2 = topo_ellipse2[1]
        radius_degrees2 = topo_ellipse2[2]
        std_dev2 = topo_ellipse2[3]
        height2 = topo_ellipse2[4]

        rsqd_array2 = np.sqrt((lon_array - central_lon2 + ax_rot*(lat_array - central_lat2))**2.+ ax_rat*(lat_array - central_lat2 - ax_rot*(lon_array - central_lon2))**2.)*np.cos(np.arctan(ax_rot))

        idx2 = (rsqd_array2 < radius_degrees2) 
        topo_array[idx2] = height2* np.exp(-(rsqd_array2[idx2]**2.)/(2.*std_dev2**2.))
    ...
    lon_0 = lons.mean()
    lat_0 = lats.mean()
    m = Basemap(lat_0=lat_0,lon_0=lon_0-1.)
