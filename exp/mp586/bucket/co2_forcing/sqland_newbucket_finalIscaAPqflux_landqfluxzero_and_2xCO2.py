...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='fccc668') #looked up commit which was used for original 2xCO2 experiment
...
exp = Experiment('sqland_newbucket_finalIscaAPqflux_landqfluxzero', codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/squareland/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/squareland/isca_qflux/ocean_qflux.nc')]
...
####### co2 experiment #######
exp = Experiment('sqland_newbucket_finalIscaAPqflux_landqfluxzero_2xCO2_spinup_361', codebase=cb)


#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/squareland/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/squareland/isca_qflux/ocean_qflux.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file='/scratch/mp586/Isca_DATA/sqland_newbucket_finalIscaAPqflux_landqfluxzero/restarts/res0361.tar.gz', num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
