...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='5d794e7')
...
exp = Experiment('two_continents_lepref1_finalIscaAPqflux_landqfluxzero', codebase=cb)
...
exp.namelist = namelist = Namelist({
    ...
    'surface_flux_nml': {
        ...
        'land_evap_prefactor': 1. # land evap = potential evap*0.7
    },
...
####### co2 experiment #######
exp = Experiment('two_continents_lepref1_finalIscaAPqflux_landqfluxzero_2xCO2_spinup_361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc'),os.path.join(GFDL_BASE,'input/two_continents/isca_qflux/ocean_qflux.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'two_continents_lepref1_finalIscaAPqflux_landqfluxzero/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
