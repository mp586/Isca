...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='7e31525')
...
exp = Experiment('flat_continents_newbucket_0qflux', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/all_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc')]
...
# create an Experiment object to handle the configuration of model parameters
# and output diagnostics
exp = Experiment('flat_continents_newbucket_0qflux_2xCO2_spinup361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/all_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/co2_doubling.nc') ]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#        'max_bucket_depth_land':.5, #Set max bucket depth over land default = 0.15 
    ...
    'mixed_layer_nml': {
        ...
        'land_albedo_prefactor' : 1.3, #What factor to multiply ocean albedo by over land
        'do_qflux' : False, #Do not use prescribed qflux formula
        'qflux_amp' : 0. # shouldn't this be false? what is 
...
# #Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'flat_continents_newbucket_0qflux/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
