...
exp = Experiment('square_South_America_frierson_insolation_newbucket_0qflux_samehcp_landocean_commit'+commit_nr, codebase=cb)
...
exp = Experiment('square_South_America_frierson_insolation_newbucket_0qflux_samehcp_landocean_plus_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'square_South_America_frierson_insolation_newbucket_0qflux_samehcp_landocean_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
