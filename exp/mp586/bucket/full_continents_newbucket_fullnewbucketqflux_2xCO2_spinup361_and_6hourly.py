...
exp = Experiment('full_continents_newbucket_fullnewbucketqflux', codebase=cb)
...
exp.namelist = namelist = Namelist({
    ...
    'idealized_moist_phys_nml': {
        ...
        'init_bucket_depth_land':.15, #Set initial bucket depth over land, default = 20, bucket is initially full 
#        'max_bucket_depth_land':.15, #Set max bucket depth over land default = 0.15 
...
# create an Experiment object to handle the configuration of model parameters
# and output diagnostics
exp = Experiment('full_continents_newbucket_fullnewbucketqflux_2xCO2_spinup361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/all_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'exp/mp586/bucket/input/full_continents_newbucket/ocean_qflux.nc'),os.path.join(GFDL_BASE,'input/co2_doubling.nc') ]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Define values for theccccb 'core' namelist
exp.namelist = namelist = Namelist({
    'main_nml': {
        'days'   : 30,
        'hours'  : 0,
    ...
    'idealized_moist_phys_nml': {
        ...
        'bucket':True, #Run with the bucket model
        'init_bucket_depth_land':.15, #Set initial bucket depth over land, default = 20, bucket is initially full 
#        'max_bucket_depth_land':.5, #Set max bucket depth over land default = 0.15 
        # src/atmos_spectral/driver/solo/idealized_moist_phys.F90
    },
...
####### Include one year where I save 6hourly data ######

cb.compile()  # compile the source code to working directory $GFDL_WORK/codebase

# create an Experiment object to handle the configuration of model parameters
# and output diagnostics
exp = Experiment('full_continents_newbucket_fullnewbucketqflux', codebase=cb) # _2xCO2_spinup361
...
# diag.add_field('dynamics', 'height', time_avg=True, files=['atmos_monthly']) # geopotential height 
# diag.add_field('atmosphere', 'rh', time_avg=True, files=['atmos_monthly']) 
# diag.add_field('dynamics', 'slp', time_avg=True, files=['atmos_monthly']) # sea level pressure
# diag.add_field('dynamics', 'zsurf', time_avg=True, files=['atmos_monthly']) # geopotential height at surface
# diag.add_field('rrtm_radiation', 'toa_sw',time_avg=True, files=['atmos_monthly'])

# diag.add_field('rrtm_radiation', 'flux_sw', time_avg=True, files=['atmos_monthly']) # net SW surface flux
# diag.add_field('rrtm_radiation', 'flux_lw', time_avg=True, files=['atmos_monthly']) # net LW surface flux
# diag.add_field('mixed_layer', 'flux_lhe', time_avg=True, files=['atmos_monthly']) # latent heat flux (up) at surface
# diag.add_field('mixed_layer', 'flux_t', time_avg=True, files=['atmos_monthly']) # sensible heat flux (up) at surface
# diag.add_field('mixed_layer', 'flux_oceanq', time_avg=True, files=['atmos_monthly'])
# diag.add_field('rrtm_radiation', 'co2', time_avg=True, files=['atmos_monthly'])

diag.add_file('atmos_6_hourly', 6, 'hours', time_units='hours')

#Tell model which diagnostics to write
diag.add_field('dynamics', 'ps', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('dynamics', 'bk')
diag.add_field('dynamics', 'pk')
diag.add_field('atmosphere', 'precipitation', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('atmosphere', 'bucket_depth', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('mixed_layer', 't_surf', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('dynamics', 'sphum', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('dynamics', 'ucomp', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('dynamics', 'vcomp', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('dynamics', 'temp', time_avg=True, files=['atmos_6_hourly'])
diag.add_field('dynamics', 'omega', time_avg=True, files=['atmos_6_hourly']) 
diag.add_field('dynamics', 'height', time_avg=True, files=['atmos_6_hourly']) # geopotential height 
diag.add_field('atmosphere', 'rh', time_avg=True, files=['atmos_6_hourly']) 
diag.add_field('dynamics', 'slp', time_avg=True, files=['atmos_6_hourly']) # sea level pressure
diag.add_field('dynamics', 'zsurf', time_avg=True, files=['atmos_6_hourly']) # geopotential height at surface
diag.add_field('mixed_layer', 'flux_lhe', time_avg=True, files=['atmos_6_hourly']) # latent heat flux (up) at surface
diag.add_field('mixed_layer', 'flux_t', time_avg=True, files=['atmos_6_hourly']) # sensible heat flux (up) at surface
