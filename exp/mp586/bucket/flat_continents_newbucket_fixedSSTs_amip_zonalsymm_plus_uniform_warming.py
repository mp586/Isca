...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='0da3f05')
...
exp = Experiment('flat_continents_newbucket_fixedSSTs_zonally_symmetric', codebase=cb)
...
exp = Experiment('flat_continents_newbucket_fixedSSTs_zonally_symmetric_plus_2pt52K_and_2xCO2_spinup_361', codebase=cb)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'flat_continents_newbucket_fixedSSTs_zonally_symmetric/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
