...
exp = Experiment('full_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_commit'+commit_nr, codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/all_continents/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/ssts_zonsymm_eqmax.nc')]
...
exp = Experiment('full_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_plus_2pt52K_and_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(input_path,'input/all_continents/land.nc'),os.path.join(input_path,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(input_path,'input/ssts_zonsymm_eqmax_unif_warmg.nc'), os.path.join(input_path,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'full_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
