...
exp = Experiment('square_Africa_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/square_Africa/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/square_Africa/isca_qflux/zero_integral/ocean_qflux.nc')]
...
####### co2 experiment #######
exp = Experiment('square_Africa_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_2xCO2_spinup_361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/square_Africa/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/square_Africa/isca_qflux/zero_integral/ocean_qflux.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'square_Africa_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
