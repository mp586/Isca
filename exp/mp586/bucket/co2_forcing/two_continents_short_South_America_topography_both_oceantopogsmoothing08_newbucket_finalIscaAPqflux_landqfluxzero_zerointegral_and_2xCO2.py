...
cb = IscaCodeBase.from_repo(repo='https://github.com/mp586/Isca.git', commit='933238f')
...
exp = Experiment('two_continents_short_South_America_topography_both_oceantopogsmoothing08_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents_short_South_America_topo_both/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/two_continents_short_South_America/isca_qflux/zero_integral/ocean_qflux.nc')]
...
#        'max_bucket_depth_land':0.5, #Set max bucket depth over land default = 0.15 
    ...
    'spectral_dynamics_nml': {
        ...
        'exponent':7.0,
        'robert_coeff':0.03,
        'ocean_topog_smoothing':0.8 # default is .93, see /src/atmos_spectral/model/spectral_dynamics.F90
...
####### co2 experiment #######
exp = Experiment('two_continents_short_South_America_topography_both_oceantopogsmoothing08_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_2xCO2_spinup_361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents_short_South_America_topo_both/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/two_continents_short_South_America/isca_qflux/zerointegral/ocean_qflux.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.set_resolution('T42', 40)
exp.run(1, restart_file=os.path.join(GFDL_DATA,'two_continents_short_South_America_topography_both_oceantopogsmoothing08_newbucket_finalIscaAPqflux_landqfluxzero/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
