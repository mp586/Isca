...
exp = Experiment('square_South_America_lepref1_0qflux_samealbedo_commit'+commit_nr, codebase=cb)
...
#diag.add_field('atimosphere', 'bucket_depth_conv', time_avg=True)
...
exp = Experiment('square_South_America_lepref1_0qflux_samealbedo_plus_2xCO2_spinup_361_commit'+commit_nr, codebase=cb)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'square_South_America_lepref1_0qflux_samealbedo_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
