...
exp = Experiment('narrow_three_newbucket_fixedSSTs_from_realworld_zonallysymm_vegetation_vegpref1', codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/narrow_three/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/sst_clim_amip_zonalsymm.nc')]
...
#Lets do a run!
# exp.run(122, restart_file=os.path.join(GFDL_DATA,'narrow_three_newbucket_fixedSSTs_from_realworld_zonallysymm_vegetation_vegpref1/restarts/res0121.tar.gz'), num_cores=NCORES)
# for i in range(123,481):
#    exp.run(i, num_cores=NCORES)

# Do CO2 run with prescribed SSTS from Isca/narrow_three_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly_2xCO2_spinup_361 starting from spun-up state of the run above

exp = Experiment('narrow_three_newbucket_fixedSSTs_from_realworld_zonallysymm_vegetation_vegpref0_plus_uniform_warming_and_2xCO2_spinup_361', codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/narrow_three/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/amip_zonsymm_uniform_warming.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'narrow_three_newbucket_fixedSSTs_from_realworld_zonallysymm_vegetation_vegpref1/restarts/res0361.tar.gz'), num_cores=NCORES)
