...
# create an Experiment object to handle the configuration of model parameters
# and output diagnostics
exp = Experiment('full_continents_newbucket_fullnewbucketqflux_4xCO2_spinup361', codebase=cb)



#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/all_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'exp/mp586/bucket/input/full_continents_newbucket/ocean_qflux.nc'),os.path.join(GFDL_BASE,'input/co2_quadrupling.nc') ]
#Tell model how to write diagnostics
diag = DiagTable()
diag.add_file('atmos_monthly', 30, 'days', time_units='days')
...
exp.run(1, restart_file=os.path.join(GFDL_DATA,'full_continents_newbucket_fullnewbucketqflux/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
