...
# exp = Experiment('flat_continents_newbucket_fixedSSTs_zonallysymm_commit'+commit_nr, codebase=cb)
...
#Lets do a run!
exp.run(1, restart_file=os.path.join(GFDL_DATA,'flat_continents_newbucket_fixedSSTs_zonallysymm_commit'+commit_nr+'/restarts/res0361.tar.gz'), num_cores=NCORES)
for i in range(2,481):
    exp.run(i, num_cores=NCORES)
