...
# Do CO2 run with prescribed SSTS from Isca/two_continents_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly + uniform warming of 2.65 K starting from spun-up state of the control fixed SST experiment (from Isca/two_continents_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly)

exp = Experiment('two_continents_newbucket_fixedSSTs_from_experiment_two_continents_newbucket_finalIscaAPqflux_landqfluxzero_zerointegral_with6hrly_plus_uniform_2pt65K_and_2xCO2_spinup_361', codebase=cb)

#Add any input files that are necessary for a particular experiment.
exp.inputfiles = [os.path.join(GFDL_BASE,'input/two_continents/land.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'),os.path.join(GFDL_BASE,'input/two_continents/prescribed_ssts_uniform.nc'), os.path.join(GFDL_BASE,'input/co2_doubling.nc')]
